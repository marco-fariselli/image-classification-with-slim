...
def create_labels_file(foreground_class_of_interest,
  ...
  """
  if type(foreground_class_of_interest) == list:
    labels_to_class_names = {0: 'background', 1: [item + ', ' for item in foreground_class_of_interest]}
  else:
  ...
  with tf.io.gfile.GFile(annotations_file, 'r') as fid:
    ...
    for category in groundtruth_data['categories']:
      if category['name'] in foreground_class_of_interest:
        foreground_class_of_interest_id.append(category['id'])
...
def _filter_annotations(annotations, image, small_object_area_threshold,
  ...
  for annotation in annotations['objects']:
    ...
    if category_id in foreground_class_of_interest_id and \
